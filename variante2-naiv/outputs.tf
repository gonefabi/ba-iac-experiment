###############################################################################
# Ausgaben – Wichtige Informationen nach dem Deployment
###############################################################################
...
output "ssh_via_windows_hint" {
  description = "SSH zur Linux-VM über die Windows-VM (Jump-Host)"
  value       = "Von der Windows-VM aus: ssh ${var.linux_admin_user}@${azurerm_network_interface.linux.private_ip_address}"
}
